...
export default mimic(1, "reduce", assert(isFunction, O => `${ O } is not a function`, assertIterator(
    async function (this: AsyncIterator<unknown>, _next: AsyncIterator<unknown, unknown, unknown>["next"], fn: (acc: unknown, item: unknown, index: number) => Promise<unknown>, accumulator?: unknown) {
        if (!(2 in arguments)) {
